...
class train_class():
    ...
    def train_model(self, model, criterion, optimizer, num_epochs=40):
        ...
        for i in range(0,all_batch,5):
            ...
            cv.imwrite('./predict_result_1/' + str(i) + '.png', res)
            cv.imwrite('./predict_result_1/GT_' + str(i) + '.png', gt_numpy)
            cv.imwrite('./predict_result_1/input_' + str(i) + '.png', 255.0*x[0,0].cpu().detach().numpy())
        ...
        # plt.show()
        # plt.close()
        torch.save(model.state_dict(), './predict_result_1/' + 'weights_1.pth' )
...
class test_class():
    def test(args):
        model = Unet(2, 2)
        model.load_state_dict(torch.load('./predict_result_1/weights_1.pth',map_location="cuda:0"))
        ...
        with torch.no_grad():
            for i, data in enumerate(test_data_loader, 0):
                ...
                cv.imwrite('./test_result_1/' + str(i) + '.png', res)
                cv.imwrite('./test_result_1/gt_' + str(i) + '.png', label)
                cv.imwrite('./test_result_1/input_' + str(i) + '.png', x[0,0].cpu().detach().numpy()*255.0)

                p, r, f1 = calculation_precison_recall().calculation(img_y_map, label)
