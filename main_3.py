...
from unet_cascade import Unet_cascade
import torchvision
...
class train_class():
    ...
    def train_model(self, model, criterion, optimizer, num_epochs=40):
        ...
        for i in range(0,all_batch,5):
            ...
            # np.save('./predict_result_1/' + str(i) + '.npy', res)
            cv.imwrite('./predict_result_3/' + str(i) + '.png', res)
            cv.imwrite('./predict_result_3/GT_' + str(i) + '.png', gt_numpy)
            cv.imwrite('./predict_result_3/input_' + str(i) + '.png', 255.0*x[0,0].cpu().detach().numpy())
        ...
        # plt.show()
        # plt.close()
        torch.save(model.state_dict(), './predict_result_3/' + 'weights_3.pth' )

        return model

    # 训练模型
    def train(self):
        model = Unet_cascade(in_ch=2, out_ch=2).to(device)
        # batch_size = 1
...
class test_class():
    def test(args):
        model = Unet_cascade(2, 2)
        model.load_state_dict(torch.load('./predict_result_3/weights_3.pth',map_location="cuda:0"))
        ...
        with torch.no_grad():
            for i, data in enumerate(test_data_loader, 0):
                ...
                # plt.imshow(res,cmap='gray')
                # plt.pause(1)
                # matplotlib.image.imsave('./test_result_3/'+ str(i) + '.png', res,cmap="gray")
                # matplotlib.image.imsave('./test_result_3/' + 'GT_' + str(i) + '.png', label, cmap="gray")
                # print('label_min_max',np.amax(label),np.amin(label))
                cv.imwrite('./test_result_3/' + str(i) + '.png', res)
                cv.imwrite('./test_result_3/gt_' + str(i) + '.png', label)
                cv.imwrite('./test_result_3/input_' + str(i) + '.png', x[0,0].cpu().detach().numpy()*255.0)
